...
def save_data(support_data, sub_set_list, path):#保存结果到txt文件
	with open(path,"w") as f:
		f.write("index  support"+"   support_data\n")
		index=1
		for item in sub_set_list:
			s=" {:<4d}  {:.3f}        {}\n".format(index, support_data[frozenset(item)], item)
			index+=1
			f.write(s)
		f.close()
	print("result saved,path is:{}".format(path))
...
class Fp_growth():
	...
	def generate_R(self,data_set, min_support, min_conf):
			...
			for freq_set in L[i]:
				for sub_set in sub_set_list:
					if sub_set.issubset(freq_set) and freq_set-sub_set in support_data:#and freq_set-sub_set in support_data
						# print(support_data[frozenset({'butter', 'newspapers'})])
		...
		return support_data, sub_set_list, rule_list
...
if __name__=="__main__":
	...
	fp=Fp_growth()
	support_data, sub_set_list, rule_list = fp.generate_R(data_set, min_support, min_conf)
	# print(sub_set_list)
	save_data(support_data, sub_set_list, save_path)
